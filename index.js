...
const lidMapCol = db.collection('lid_map') // מיפוי LID -> מספר טלפון
...
let BLOCKED_PHONES = []
let FAMILY_PHONES = []
let lidToPhone = {}  // מיפוי lid -> phone
let phoneToLid = {}  // מיפוי phone -> lid
...
async function addReminder(phone, day, time, resolvedDate) {
  try {
    // מחק תזכורות קודמות של אותו מספר
...
// טעינת slot counts מ-Firebase בעלייה
...
// ========== LID Detection - לומד מיפויים חדשים ==========
async function tryLearnLid(sock, sender) {
  // אם זה LID ועוד לא מוכר - ננסה ללמוד
  if (sender.includes('@lid')) {
    const lid = sender.replace('@lid', '')
    if (!lidToPhone[lid]) {
      // ננסה לקבל פרטי contact
      try {
        ...
        if (result && result.jid && result.jid.includes('@s.whatsapp.net')) {
          ...
          console.log('LID mapping learned: ' + lid + ' -> ' + phone)
        }
      } catch {}
    }
  }
...
async function initFirebaseData() {
  // טען מספרים חסומים ראשוניים אם ה-collection ריק
...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // ========== לימוד מיפוי LID אוטומטי ==========
      await tryLearnLid(sock, sender)

      // ========== בדיקת participant (אם יש) לגיבוי מיפוי ==========
      if (msg.key.participant) {
        const participantPhone = msg.key.participant.replace('@s.whatsapp.net', '')
        if (sender.includes('@lid') && participantPhone.match(/^972/)) {
          const lid = sender.replace('@lid', '')
          await saveLidMapping(lid, participantPhone)
        }
      }

      // ========== משפחה - ג'ימי שותק לחלוטין, רק מעביר ליאיר ==========
