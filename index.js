...
// פותר את בעיית ההתנתקות — auth_info נשמר ב-DB ולא בקובץ מקומי
...
async function useSupabaseAuthState() {
  // טוען auth קיים מ-Supabase
  let authData = await loadAuthFromSupabase()

  // אם אין — יוצר תיקיית auth_info ריקה (baileys ייצור QR)
  if (!authData) {
    console.log('אין auth שמור — ממתין ל-QR')
    try { mkdirSync('auth_info', { recursive: true }) } catch {}
  } else {
    // שומר לקובץ מקומי זמני כדי ש-baileys יוכל לטעון
    try {
      mkdirSync('auth_info', { recursive: true })
      // כותב את כל הקבצים מה-DB לתיקייה
      if (authData.files) {
  ...
  // עוטף את saveCreds כדי לשמור גם ל-Supabase
  const saveCredsAndSync = async () => {
    await saveCreds()
    // קורא את הקבצים מהתיקייה ושומר ל-Supabase
    try {
      const files = {}
      const authFiles = ['creds.json', 'app-state-sync-key-GLOBAL.json']
      for (const f of authFiles) {
        try {
          if (existsSync(`auth_info/${f}`)) {
            files[f] = readFileSync(`auth_info/${f}`, 'utf-8')
          }
        } catch {}
      }
      // שמור גם pre-keys וsession keys
      try {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if (isBadMac || isSessionError) {
        ...
        badMacCount++
        if (badMacCount >= 3) {
          console.error('🔴 3+ Bad MAC errors — מוחק auth_info ומ-Supabase, צריך QR חדש!')
