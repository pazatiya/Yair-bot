...
async function addToGoogleCalendar(day, time, customerPhone) {
  try {
    const auth = getGoogleAuth()
    if (!auth) { console.log('גוגל יומן לא מוגדר - מדלג'); return }

    const { now } = getIsraeliDateInfo()
    const { today, tomorrow } = getIsraeliDateInfo()
    const isToday = day === today
    const isTomorrow = day === tomorrow
    const eventDate = resolveToDate(day, isToday, isTomorrow)
    if (!eventDate) return
    ...
    await calendar.events.insert({
      ...
      requestBody: {
        summary: 'תור אצל יאיר - ' + customerPhone.replace('@s.whatsapp.net',''),
        description: 'תור שנקבע דרך הבוט\nטלפון: ' + customerPhone.replace('@s.whatsapp.net',''),
        start: { dateTime: eventDate.toISOString(), timeZone: 'Asia/Jerusalem' },
        end: { dateTime: endDate.toISOString(), timeZone: 'Asia/Jerusalem' },
      }
    })
    console.log('נוסף ליומן גוגל: ' + day + ' ' + time)
  } catch (err) {
    console.error('שגיאה בגוגל יומן:', err?.message)
  }
}
  ...
  if (reply.includes('APPOINTMENT_BOOKED')) {
    ...
    if (dayM && timeM) {
      ...
      if (isSlotAvailable(day, time)) {
        ...
        // הוספה לגוגל יומן
        await addToGoogleCalendar(day, time, userPhone)
