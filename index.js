...
async function useSupabaseAuthState() {
  ...
  const state = {
    ...
    keys: {
      get: async (type, ids) => {
        const result = {}
        for (const id of ids) {
          const key = allKeys[type + '_' + id] || await loadKey(type + '_' + id)
      ...
      set: async (data) => {
        ...
        for (const [type, typeData] of Object.entries(data)) {
          for (const [id, value] of Object.entries(typeData || {})) {
            ...
            if (value) {
              allKeys[keyId] = value
              promises.push(saveKey(keyId, value))
            } else {
              delete allKeys[keyId]
              promises.push(deleteKey(keyId))
            }
          }
        }
  ...
  return { state, saveCreds: saveCredsWrapper }
}
