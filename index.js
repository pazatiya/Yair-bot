  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (sender.includes('@lid')) {
        const resolved = await resolveLidToPhone(sender)
        if (resolved) {
          console.log('הודעה מ-' + sender + ' (=' + resolved + '): ' + text)
        } else {
          console.log('הודעה מ-' + sender + ' (LID לא ממופה!): ' + text)
        }
      } else {
        console.log('הודעה מ-' + sender + ': ' + text)
      }

      // ========== בדיקת senderKeyDistributionMessage / participant לגיבוי ==========
