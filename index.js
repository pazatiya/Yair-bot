...
const OWNER_PHONE2 = '972547734708@s.whatsapp.net'
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if (isBadMac || isSessionError) {
        console.error('⚠️ Bad MAC! מוחק session keys ומתחבר מחדש...')
        badMacCount++
        if (badMacCount >= 10) {
          console.error('🔴 10+ Bad MAC errors — מוחק auth לגמרי, צריך QR חדש!')
          try { await supabase.from('bot_auth').delete().neq('id', 'dummy') } catch {}
          badMacCount = 0
        } else {
          // מוחק רק session keys, שומר creds
          try {
            await supabase.from('bot_auth').delete().like('id', 'key_%')
            console.log('מחקתי session keys')
          } catch {}
        }
