...
async function useSupabaseAuthState() {
  ...
  // תמיד מוחקים auth_info ישן לפני שמתחילים — מונע logged out loop
  try { rmSync('auth_info', { recursive: true, force: true }) } catch {}
  try { mkdirSync('auth_info', { recursive: true }) } catch {}

  if (!authData) {
    console.log('אין auth שמור — ממתין ל-QR חדש')
  } else {
    try {
      if (authData.files) {
      ...
      console.log('auth נטען מ-Supabase — מתחבר...')
    } catch (err) {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if (isBadMac || isSessionError) {
        ...
        try { rmSync('auth_info', { recursive: true, force: true }) } catch {}
        if (loggedOutCount >= 2) {
          console.log('🔴 logged out חוזר — מוחק auth מ-Supabase, צריך QR חדש')
          try { await supabase.from('bot_auth').delete().eq('id', 'main') } catch {}
          loggedOutCount = 0
        } else {
          console.log('מנסה להתחבר מחדש לפני מחיקת auth...')
        }
