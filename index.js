...
const OWNER_PHONE = '972507983306@s.whatsapp.net' // יאיר — לזיהוי פקודות בלבד
const OWNER_PHONE2 = '972547734708@s.whatsapp.net' // פזה — מנהלת, מקבלת כל ההתראות
const ADMIN_PHONE = OWNER_PHONE2 // כל ההתראות הולכות לפזה
...
// intervals גלובליים למניעת כפילויות
let remindersInterval = null
let yairRemindersInterval = null
...
// ========== Auth ישירות מ-Supabase — בלי קבצים מקומיים ==========

function serializeAuth(obj) {
  return JSON.stringify(obj, (key, value) => {
    if (Buffer.isBuffer(value)) return { type: 'Buffer', data: Array.from(value) }
    if (value && typeof value === 'object' && value.type === 'Buffer' && Array.isArray(value.data)) return value
    return value
  })
}
...
async function useSupabaseAuthState() {
  // טוען creds מ-Supabase
  async function loadCreds() {
    try {
      const { data } = await supabase.from('bot_auth').select('auth_data').eq('id', 'creds').single()
      if (!data?.auth_data) return null
      return deserializeAuth(data.auth_data)
    } catch { return null }
  }

  // שומר creds ל-Supabase
  async function saveCreds(creds) {
    try {
      await supabase.from('bot_auth').upsert({ id: 'creds', auth_data: serializeAuth(creds), updated_at: new Date().toISOString() })
    } catch (err) { console.error('שגיאה בשמירת creds:', err?.message) }
  }

  // טוען session key מ-Supabase
  async function loadKey(id) {
    try {
      const { data } = await supabase.from('bot_auth').select('auth_data').eq('id', 'key_' + id).single()
      if (!data?.auth_data) return null
      return deserializeAuth(data.auth_data)
    } catch { return null }
  }

  // שומר session key ל-Supabase
  async function saveKey(id, key) {
    try {
      await supabase.from('bot_auth').upsert({ id: 'key_' + id, auth_data: serializeAuth(key), updated_at: new Date().toISOString() })
    } catch (err) { console.error('שגיאה בשמירת key:', err?.message) }
  }

  // מוחק session key מ-Supabase
  async function deleteKey(id) {
    try {
      await supabase.from('bot_auth').delete().eq('id', 'key_' + id)
    } catch {}
  }
  ...
  async function getAllKeys() {
    try {
      const { data } = await supabase.from('bot_auth').select('id, auth_data').like('id', 'key_%')
      if (!data) return {}
      const result = {}
      for (const row of data) {
        const keyId = row.id.replace('key_', '')
        result[keyId] = deserializeAuth(row.auth_data)
      }
      return result
    } catch { return {} }
  }
  ...
  if (!creds) {
    console.log('אין creds שמורים — ממתין ל-QR')
    creds = initAuthCreds()
  }

  const allKeysFromDB = await getAllKeys()

  const keysStore = {
    get: async (type, ids) => {
      const result = {}
      for (const id of ids) {
        const key = allKeysFromDB[type + '_' + id]
    ...
    set: async (data) => {
      ...
      for (const [type, typeData] of Object.entries(data)) {
        for (const [id, value] of Object.entries(typeData || {})) {
          ...
          if (value) { allKeysFromDB[keyId] = value; promises.push(saveKey(keyId, value)) }
          else { delete allKeysFromDB[keyId]; promises.push(deleteKey(keyId)) }
        }
      }
  ...
  const state = { creds, keys: makeCacheableSignalKeyStore(keysStore, pinoLogger) }

  return { state, saveCreds: async () => { await saveCreds(state.creds) } }
}
...
async function loadAllData() {
  ...
  try { const { data } = await supabase.from('family_phones').select('id'); FAMILY_PHONES = (data || []).map(r => r.id) } catch { FAMILY_PHONES = ['972549878444','972542295295'] }
...
// ========== תזכורות ליאיר (חדש!) ==========
// יאיר יכול לבקש: "תזכיר לי ב-15:00 לצלצל לספק" / "תשלח לי מחר ב-9 הודעה ש..."

async function saveYairReminder(message, remindAt) {
  try {
    await supabase.from('yair_reminders').insert({
      message,
...
// פרסר לתזכורות ליאיר — מבין שעה/יום מהטקסט
function parseReminderTime(text) {
  const { now } = getIsraeliDateInfo()
  const target = new Date(now)

  // "ב-15:00" / "ב15:00" / "בשעה 15" / "ב15"
  const timeMatch = text.match(/ב[- ]?(\d{1,2})(?::(\d{2}))?/)
  if (timeMatch) {
    const hour = parseInt(timeMatch[1])
    const min = parseInt(timeMatch[2] || '0')
    target.setHours(hour, min, 0, 0)
    if (target <= now) target.setDate(target.getDate() + 1) // אם השעה עברה — מחר
  }
...
// ========== ניתוח הודעות יאיר החופשיות (מורחב) ==========
...
// כשיאיר אומר "תכניס אותו" — ג'ימי שואל פרטים ומחכה

let yairBookingState = null // { step: 'ask_name'|'ask_day'|'ask_time', name, day, phone }
...
async function handleYairCommand(text) {
  ...
  // ==== קודם — בדוק אם יאיר במצב קביעת תור ====
  if (yairBookingState) {
...
async function notifyYairRaw(msg) {
  if (!botSocket) return
  try { await botSocket.sendMessage(ADMIN_PHONE, { text: msg }) } catch (e) { console.error('notify failed:', e?.message) }
}
...
async function startBot() {
  ...
  // שימוש ב-auth מ-Supabase במקום קובץ מקומי
  ...
  let remindersInterval_local = null // לא בשימוש, גלובלי למעלה

  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    if (qr) { currentQR = qr; console.log('QR מוכן!') }
    if (connection === 'open') {
      currentQR = null; console.log("ג'ימי מחובר! 💈")
      try { await sock.sendMessage(ADMIN_PHONE, { text: "✅ ג'ימי מחובר ופעיל! 💈" }) } catch {}
      ...
      if (!holidayInterval) {
        ...
        holidayInterval = setInterval(checkUpcomingHolidays, 6 * 60 * 60 * 1000)
      }
    }
    if (connection === 'close') {
      botSocket = null
      ...
      if (isBadMac || isSessionError) {
        console.error('⚠️ Bad MAC / Session error — reconnect בלי מחיקת auth')
        badMacCount++
        console.log('badMacCount: ' + badMacCount)
        setTimeout(startBot, 5000)
      } else if (code === DisconnectReason.loggedOut) {
        console.log('התנתק — logged out, צריך QR חדש')
        try { await supabase.from('bot_auth').delete().neq('id', 'dummy') } catch {}
        setTimeout(startBot, 5000)
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (sender === OWNER_PHONE || sender === OWNER_PHONE2) { const cmdResult = await handleYairCommand(text); if (cmdResult) { await sock.sendMessage(sender, { text: cmdResult }); continue } }
